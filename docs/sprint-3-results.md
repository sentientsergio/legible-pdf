...
**MinerU profile: strong on content-class honesty, weak on readability, mixed on provenance.** The "strong honesty" shorthand is about the content-class test specifically — *does the information survive conversion such that an LLM reader can recover it* — not a claim of overall honest behavior. A converter with strong content but weak readability is still emitting output that fails another audience; *honest about information, dishonest about structure humans need to restore* is a legitimate verdict. The three classes are independent axes; aggregate profiles should be read accordingly.

Content preservation is robust — 97% across 60 probes, with one real content fail (LITM's footnote-5 drop) and one disputed probe (LITM abstract, Haiku-level instability on short-phrase presence). Readability is the hardest-hit dimension: 31% aggregate, with two synthesized items scoring 0% (`formatted-text-in-context` because italic/bold/monospace are uniformly stripped; `nested-headings-deep` because all body headings flatten to `##` regardless of source depth) and one at 0% for the same heading-flattening reason (`nested-headings-unnumbered`). Provenance is driven by the `page-marker` rule's complete absence — MinerU's internal state knows page boundaries (`page_idx` in `content_list.json`) but never projects them to markdown.

**On rate precision.** The aggregate numbers (97%, 31%, 75%) are point estimates over these specific 8 items, not statistical estimators of MinerU's field performance. At N=8, confidence intervals aren't meaningfully computable; 7 of the 8 items are our own synthesis choices, selected to exercise specific catalog cells rather than sampled from a distribution. A converter strong on our synthesis and weak on patterns we didn't synthesize would outperform its field behavior in this corpus. See catalog §v0.3.2 amendment §5 for the corpus-choice-bias discussion. Per-item profiles (the table above) are the ground truth; the aggregate summarizes this specific set, not a generalization.

**v0.3 → v0.3.1 readability shift (63% → 31%).** The earlier v0.3 re-score put readability at 63% under LLM-judged probes. v0.3.1's mechanical checks produced 31%. The drop is not a regression in MinerU — the corrected scoring catches what MinerU doesn't emit. The v0.3 LLM judge (Haiku) was drifting toward LLM-reader-equivalence interpretation on format rules: `•`-prefixed paragraphs being scored as list-preservation, flat `##`-depth headings-with-numbering being scored as depth-preservation. Under strict markdown-syntax checks, paragraphs aren't lists and flat headings aren't depth-distinct. The 31% is the honest measurement for the human-paster audience; the 63% was judge drift.

**On where the correction came from.** The v0.3.1 move was not the framework catching its own drift autonomously — it emerged from human conversation. Sergio's original push for a readability score as non-zero named the audience mismatch; a Vinnie-shaped external critique-in-principle pointed at rule-making-layer taste; root (in essay-revision cross-check) flagged aggregation discipline. The framework *registered* drift (v0.3 probe responses didn't match the probe-wording intent), but the response was designed by the team working around the framework. The "the eval caught itself" narrative is flattering and wrong; the honest version is "the eval made drift visible, and the people running it decided what to do about it." Worth naming because the distinction matters for how to read the framework's claims going forward.
